// PDFから予定表データを抽出してCSVに変換するスクリプト
// CSVのカラムは以下の通り
// date.mon, date.day, event_hs, change, cafe, study_room, grade1_timetable_1, grade1_timetable_2, ..., grade1_timetable_7, grade2_timetable_1, ..., grade3_timetable_7
...
async function main() {
  ...
  if (!pdfPath) {
    console.error("使い方: node index2.mjs <PDFファイルパス>");
    process.exit(1);
  }

  const logger = (levels, msg) => {
    if (levels.includes(params.log_level)) console.log(msg);
  };
...
async function procPage(logger, page) {
  ...
  fs.writeFileSync(
    `logs/${params.log_records_name}_page${page.pageNumber}.json`,
  ...
  fs.writeFileSync(
    `logs/${params.log_merged_records_name}_page${page.pageNumber}.json`,
    JSON.stringify(merged_records, null, 4),
    "utf-8",
  );
...
function split_records(logger, contents) {
  // Y座標降順ソート(物理的に上から下へ)
  const contents_y_desc = contents.sort((a, b) => b.y_coord - a.y_coord);
...
function col_match(logger, item) {
  if (params.header_under_border < item.y_coord) {
    logger(
      ["trace", "warn"],
      `ヘッダー下限ボーダーによりアイテムを無視: ${JSON.stringify(item)}`,
    );
    return null;
  }
  if (
    params.col_ranges.date.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.date.max
  ) {
    ...
    logger(
      ["trace"],
      `日付カラムマッチ: ${mon}/${day} : ${JSON.stringify(item)}`,
    );
    return {
      date: { mon, day },
    };
  ...
  if (
    params.col_ranges.change.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.change.max
  ) {
    let res = item.str;
  ...
  if (
    params.col_ranges.event_hs.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.event_hs.max
  ) {
    logger(["trace"], `行事カラムマッチ: ${JSON.stringify(item)}`);
    return { event_hs: item.str };
  }
  if (
    params.col_ranges.grade.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.grade.max
  ) {
    let grade = null;
    switch (item.str) {
      case "①":
    ...
    logger(
      ["trace"],
      `学年カラムマッチ: ${grade}年次 : ${JSON.stringify(item)}`,
    );
    return { grade };
  }
  if (
    params.col_ranges.cafe.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.cafe.max
  ) {
    let open = false;
    if (item.str === "〇") {
      open = true;
    ...
    logger(
      ["trace"],
      `カフェテリアカラムマッチ: Open=${open} : ${JSON.stringify(item)}`,
    );
    return { cafe: open };
  }
  if (
    params.col_ranges.study_room.min <= item.x_coord &&
    item.x_coord <= params.col_ranges.study_room.max
  ) {
    let avable = null;
    if (item.str == "〇") {
      avable = true;
    ...
    logger(
      ["trace"],
      `自習室カラムマッチ: Open=${avable} : ${JSON.stringify(item)}`,
    );
  ...
  logger(
    ["trace", "warn"],
    `いずれのカラムにもマッチしなかったアイテムを無視: ${JSON.stringify(item)}`,
  );
  return null;
}
...
function merge_records_by_date(records) {
  ...
  for (let i = 0; i < records.length; i += 3) {
    ...
    if (
      row_1.date.mon !== row_2.date.mon ||
      row_1.date.day !== row_2.date.day ||
      row_1.date.mon !== row_3.date.mon ||
      row_1.date.day !== row_3.date.day
    ) {
    ...
    if (
      ...
      row_2.grade &&
      row_3.grade &&
      (row_1.grade !== 1 || row_2.grade !== 2 || row_3.grade !== 3)
    ) {
    ...
    const change = merge_cells(
      [row_1.change, row_2.change, row_3.change],
      "\n",
    );

    const event_hs = merge_cells(
      [row_1.event_hs, row_2.event_hs, row_3.event_hs],
      "\n",
    );
    ...
    const grade_timetables = {
      1: {},
      2: {},
      3: {},
    };
    for (let j = 1; j <= 7; j++) {
      grade_timetables[1][j] = row_1[`timetable_${j}`] || null;
      grade_timetables[2][j] = row_2[`timetable_${j}`] || null;
      grade_timetables[3][j] = row_3[`timetable_${j}`] || null;
    }
...
function to_csv(records) {
  ...
  for (const record of records) {
    ...
    for (let grade = 1; grade <= 3; grade++) {
      for (let i = 1; i <= 7; i++) {
        line_elems.push(record.grade_timetables[grade][i] || "");
      }
    }
    lines.push(
      line_elems
        .map((elem) => `"${String(elem).replaceAll('"', '""')}"`)
        .join(","),
    );
  }
  return lines.join("\n");
